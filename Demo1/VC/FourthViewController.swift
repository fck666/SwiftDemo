...
class FourthViewController: UIViewController {
    ...
    func setUpCollection()  {
        ...
        let collection = UICollectionView(frame: CGRect(x: view.bounds.maxX/4, y: 50, width: view.bounds.width-view.bounds.width/4+1, height: view.bounds.height), collectionViewLayout: layout)
...
extension FourthViewController :UICollectionViewDelegate,UICollectionViewDataSource,UITableViewDelegate,UITableViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.config(text: data.departmentGroup[a].departmentName, image: data.departmentGroup[a].departmentImage)
