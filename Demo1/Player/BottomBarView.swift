...
class BottomBarView: UIView {
    ...
    fileprivate(set) lazy var playedTimeLabel: UILabel = {
        ...
        playedTimeLabel.textColor = UIColor.red
    ...
    fileprivate(set) lazy var totalTimeLabel: UILabel = {
        ...
        totalTimeLabel.textColor = UIColor.red
    ...
    fileprivate(set) lazy var orientationButton: UIButton = {
        ...
        orientationButton.tintColor = UIColor.red
        orientationButton.setImage(UIImage.init(named: "go_landscape")?.from(tintColor: UIColor.red), for: .normal)
        orientationButton.setImage(UIImage.init(named: "go_portrait")?.from(tintColor: UIColor.red), for: .selected)
    ...
    func adjustUI() {
        if (UIDevice.current.systemVersion as NSString).floatValue < 8.3 {
            if let image = slider.currentThumbImage {
                let img = image.from(tintColor: UIColor.red)
